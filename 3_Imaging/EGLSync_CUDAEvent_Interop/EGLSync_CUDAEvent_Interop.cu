...
// Using EGLSync - CUDA Event interop one can achieve synchronization on GPU itself for GL-EGL-CUDA operations 
// instead of blocking CPU for synchronization.
// This app requires GLES 3.2 or higher
...
#define CUDA_SAFE_CALL_NO_CLEANUP(call, err) do {                           \
...
__global__ void verify_and_update_kernel(CUsurfObject write, CUsurfObject read, char expected, char newval, int width, int height);
...
void checkSync(int argc, char **argv)
{
    ...
    EGLImageKHR eglImage1 = eglCreateImageKHR(eglDisplayHandle, eglCtx, EGL_GL_TEXTURE_2D_KHR, (EGLClientBuffer)(intptr_t)tex[GL_READ], eglImgAttrs);
    ...
    EGLImageKHR eglImage2 = eglCreateImageKHR(eglDisplayHandle, eglCtx, EGL_GL_TEXTURE_2D_KHR, (EGLClientBuffer)(intptr_t)tex[GL_WRITE], eglImgAttrs);
    ...
    if (status != CUDA_SUCCESS) {
        ...
        printf("cuGraphicsEGLRegisterImage Passed, writeResource created with texture 1\n");
    }

    status = cuGraphicsSubResourceGetMappedArray(&writeArray, writeResource, 0, 0);
    if (status != CUDA_SUCCESS) {
        printf ("cuGraphicsSubResourceGetMappedArray failed for writeResource with texture 1\n");
        cleanup(FAILURE);
    }
    ...
    if (status != CUDA_SUCCESS) {
        ...
        printf("cuGraphicsEGLRegisterImage Passed, readResource created with texture 2\n");
    }
...
void checkSyncOnCPU(void) 
{
    ...
    for (z = 0; z < itr; z++)
    {
        ...
        verify_and_update_kernel<<<(width*height)/256, 256>>>(writeSurface, readSurface, expectedData, newData, width, height);
...
/*
    ...
    we make use of EGLSync, CUDA Event and cuStreamWaitEvent, eglWaitSyncKHR to achieve the synchronization
    due to this CPU is not blocked for any synchronization needed between GL-EGL & CUDA operations
    all synchronizations happens on the GPU only.
*/
void checkSyncOnGPU(EGLDisplay dpy) 
{
    int z = 0;
    ...
    /*
        ...
        3. Using cuStreamWaitEvent() we wait in GPU for the GLES to finish texture copy.
        4. CUDA kernel verfiy_and_update_kernel verifies if the copied data by GLES is correct,
           and it updates the buffer with new values.
        5. This is followed by eglWaitSyncKHR() which waits for the cuda kernel to finish,
           so that in the next iteration GLES can perform the copying of the updated buffer to write texture,
    */
    for(z = 0; z < itr; z++) {
        ...
        // Verifies the values in readSurface which is copied by glCopyImageSubData()
        // And writes value of newData into writeSurface
        verify_and_update_kernel<<<(width*height)/256, 256, 0, stream>>>(writeSurface,readSurface, expectedData, newData, width, height);
...
__global__ void verify_and_update_kernel(CUsurfObject write, CUsurfObject read, char expected, char newval, int width, int height)
{
    ...
    if ( x < width && y < height) 
    {
        ...
        {
            printf("Mismatch found in values read[0]= %u read[1]= %u read[2]= %u read[3]= %u expected is %u\n",check.x, check.y, check.z, check.w, expected);
            numErrors++;
            return;
        }
